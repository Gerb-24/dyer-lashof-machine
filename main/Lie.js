import { cartan, nishida, adem } from "./Lie_functions.js";
          ...
          for (let node of operations) {
            ...
            if (node instanceof Operation) {
              operationsList = Array.from(
                { length: maxDim - node.degree - 2 * node.power - 1 },
                (_, i) => new Operation(i + 2 * node.power + 1, node)
              );
            } else {
              operationsList = Array.from(
                { length: maxDim - node.degree - node.degree },
